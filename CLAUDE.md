...
5. **Every provider lands in the same account shape, and nothing is a special case at the presentation layer.** A provider whose main quota is weekly uses `meter_id: "weekly_all"` and `label: "Weekly · all models"`; a 5h-class throttle uses `session`. `active` and `logins` mean "the Claude login on this machine" and render on Claude rows only. If a renderer needs an `if provider == ...`, the normalization is wrong.
